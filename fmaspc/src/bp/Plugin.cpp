...
bool OutputPlugin::generate(OutputAdapter *adapter) {
  ...
  for (const auto &bp : symbols->getBreakpoints()) {
    bool valid = true;
    uint64_t address = bp.reference->resolve(symbols, valid);
    if (!valid) {
      project->log().warn() << "Unable to resolve symbol using resolve() routine: " << bp.reference->asString();
      address = symbols->getResolved(bp.reference->asString());
    }
    os << std::hex << std::setw(6) << std::setfill('0') << (address & 0xFFFFFF) << ":x" << (bp.notifyOnly ? "n" : "") << ":smp#" << bp.comment << std::endl;
